...
//    }
    ...
    @IBAction func startEventBtn(_ sender: Any) {
        ...
        let OldGirlLink = self.imageGirlLink.replacingOccurrences(of: "https://photo.gachmen.org", with: "/var/www/build_futurelove", options: .literal, range: nil)
        let OldBoyLink = self.imageboyLink.replacingOccurrences(of: "https://photo.gachmen.org", with: "/var/www/build_futurelove", options: .literal, range: nil)
    ...
    func uploadGenLoveByImages(isNam:Bool,image_Data:UIImage,completion: @escaping ApiCompletion){
        ...
        APIService.shared.UploadImagesToGenRieng("https://api.funface.online/upload-gensk/" + String(AppConstant.userId ?? 0) + checkNamNu, ImageUpload: image_Data,method: .POST, loading: true){data,error in
            completion(data, nil)
        }
    }
        ...
        if self.currentImageType == .boy {
            ...
            self.uploadGenLoveByImages(isNam: true, image_Data: image){data,error in
                if let data = data as? String{
                    let OldBoyLink = data.replacingOccurrences(of: "/var/www/build_futurelove", with: "https://photo.gachmen.org", options: .literal, range: nil)
            ...
            self.uploadGenLoveByImages(isNam: false, image_Data: image){data,error in
                if let data = data as? String{
                    let OldGirlLink = data.replacingOccurrences(of: "/var/www/build_futurelove", with: "https://photo.gachmen.org", options: .literal, range: nil)
