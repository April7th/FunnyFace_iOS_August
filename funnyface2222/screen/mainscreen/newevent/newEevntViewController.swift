...
class newEevntViewController: BaseViewController {
    ...
    @IBAction func saveAction(_ sender: Any) {
        let linkImagePro = self.linkImage.replacingOccurrences(of: "https://photo.gachmen.org", with: "/var/www/build_futurelove", options: .literal, range: nil)
...
//        }
            ...
            let linkImagePro = self.linkImage.replacingOccurrences(of: "/var/www/build_futurelove", with: "https://photo.gachmen.org", options: .literal, range: nil)
            print(linkImagePro)
            let url = URL(string:self.listImage[indexPath.row])
                       let processor = DownsamplingImageProcessor(size: self.imageAvatar.bounds.size)
                                    |> RoundCornerImageProcessor(cornerRadius: 20)
